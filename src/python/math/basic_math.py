...
def prime_factors(n: int) -> list:
    ...
    The Algorithm:
    First we deal with all the composite numbers in step 1 then prime numbers in step 2.
    ...
    In step 2 of the above algorithm, we run a loop and do:
        a) Find the least prime factor i (must be less than √n,)
        b) Remove all occurrences i from n by repeatedly dividing n by i.
        c) Repeat steps a and b for divided n and i += 2. The steps a and b are repeated till n becomes either 1 or a prime number.
...
def number_of_divisors(n: int) -> int:
    ...
    2. Once completely reduced, we multiply it to div, meaning that as many times as we could reduce n by 2 i.e. as many even divisors exist for n.
    3. Then similar to step 2 of prime_factors we check for prime numbers and do following in loop up to sqrt of remaining n:
